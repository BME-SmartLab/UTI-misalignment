...
for speaker in speakers:
    if not os.path.exists('measures/' + speaker + '_CW_SSIM_avg.pkl'):
        ...
        pickle.dump(CW_SSIM_all, open('measures/' + speaker + '_CW_SSIM_avg.pkl', 'wb'))
    else:
        CW_SSIM_all = pickle.load(open('measures/' + speaker + '_CW_SSIM_avg.pkl', 'rb'))

    print(speaker, ', len:', len(CW_SSIM_all), ', CW_SSIM min/max: ', np.min(CW_SSIM_all), np.max(CW_SSIM_all))
    
    # visualize results
    # vmax set according to manual checking 
    plt.imshow(CW_SSIM_all, cmap='Greys_r', vmin=0., vmax=0.5)
    plt.title('CW_SSIM, ' + speaker)
    plt.xlabel('Time (utterance no.)')
    plt.ylabel('Time (utterance no.)')
    plt.savefig('figs/' + speaker + '_CW_SSIM_matrix.png')
    plt.close()
